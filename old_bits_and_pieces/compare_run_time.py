...
for option in which_options:
    file1 = open('output/testfiles_poly_nmin%i_nmax%i_nstep%i_%s_seed%i_layers_m2.txt' % (n_min,n_max,n_step,option,seed),'r')
    lines1 = file1.read().splitlines()[1:]
    file1.close()

    file2 = open('output/testfiles_table_nmin%i_nmax%i_nstep%i_%s_seed%i_layers_py.txt' % (n_min,n_max,n_step,option,seed),'r')
...
for i in range(len(which_options)):
    ...
    ax.semilogy(ns,mean_values1,lss[i],color='r')
    ax.semilogy(ns,mean_values2,lss[i],color='b')
...
plt.savefig('figures/comparison_runtime_nmin%i_nmax%i_nstep%i_%s_seed%i.pdf' % (n_min,n_max,n_step,option,seed),bbox_inches = "tight")
